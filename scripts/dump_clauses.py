...
def _get_conn():
    """매 호출마다 새 DB 연결 (봇에서 안전하게 사용 가능)."""
    return sqlite3.connect(str(DB_PATH))


def get_signal(signal_id=None, conn=None):
...
def gather_clauses(issue, target, year_week, conn=None):
    _conn = conn or _get_conn()
    try:
...
def dump_to_string(signal_id=None, year_week=None, issue=None, target=None):
    ...
    try:
        if year_week and issue and target:
            ...
            sig = get_signal(signal_id, conn=conn)
            if not sig:
                return None, "신호 없음"
            sig_id, year_week, issue, target, score = sig

        clauses, date_from, date_to = gather_clauses(
            issue, target, year_week, conn=conn
        )
