...
# ── 기사 생성 프롬프트 (악시오스/슬로우뉴스 스타일) ──
NARRATE_PROMPT = """당신은 국회 출입 전문 기자입니다.
아래는 {year_week_kr} 국회 회의록에서 추출한 발언들입니다.

이 발언들을 바탕으로, 아래 4개 섹션으로 구성된 브리핑을 써주세요.

## 형식 (이 순서와 소제목을 반드시 지킬 것)

**핵심 쟁점**
이번 주 {issue} 분야에서 무슨 일이 벌어졌는지. 2~3문장으로 사건의 골격을 잡는다. 누가 무엇을 문제 삼았고, 상대가 어떻게 반응했는지. 발언을 직접 인용해서 생생하게.

**왜 중요한가**
이 논의가 단순한 말싸움이 아니라 왜 주목해야 하는지. 정책적·정치적·제도적 맥락. 3~4문장.

**충돌 지점**
여야, 또는 질문자-답변자 사이에 견해가 갈리는 구체적인 지점. 양측 발언을 대비시켜서 보여준다. 직접 인용 필수.

**전망**
이 이슈가 어디로 갈 것인지. 다음에 뭘 봐야 하는지. 1~2문장.

## 작성 원칙
- 발언을 "..." 형태로 직접 인용. 발언자 이름과 소속(당, 직함)을 밝힌다.
- 수치, 점수, 신호 강도 같은 기계적 메타데이터는 절대 쓰지 않는다.
- 관료적 문장("~에 대하여 논의가 이루어졌다") 대신 능동태와 구어체를 쓴다.
- 분량: 전체 1000~1500자.
...
## 출력
위 4개 섹션으로 구성된 브리핑 본문만 쓰세요. 제목이나 메타데이터 없이."""
