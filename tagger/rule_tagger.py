...
SUBSTANTIVE_ACTS = [
    ...
    # ── v3 추가: 미태깅 77% 커버리지 확대 ──
    ("수락", re.compile(
        r'(^예[,.]?\s*(알겠|그렇|네)|^네[,.]?\s*(알겠|그렇)|'
        r'알겠습니다|그렇게\s*하겠습니다|잘\s*알겠습니다|'
        r'말씀\s*잘\s*알겠|숙지\s*하겠|반영\s*하겠|'
        r'수용\s*하겠|참고\s*하겠)'), 0.6),
    ("정보제공", re.compile(
        r'(현재\s*[\d,]+|총\s*[\d,]+|약\s*[\d,]+|'
        r'전년\s*(대비|比)|지난해|올해\s*[\d,]+|'
        r'[\d,]+억|[\d,]+만\s*원|[\d,]+%|'
        r'[\d,]+명|[\d,]+건|[\d,]+개\s*(사업|기관|시설|학교|병원)|'
        r'현재\s*기준|현황을\s*말씀|통계를\s*보면|자료에\s*따르면)'), 0.55),
    ("의례", re.compile(
        r'(수고\s*(하셨|많으셨|하고\s*계시)|'
        r'감사\s*(합니다|드립니다)\s*$|존경하는\s*[가-힣]+\s*(위원장|의장)|'
        r'위원님\s*감사|장관님\s*감사|'
        r'이상\s*(으로|입니다)|'
        r'마치겠습니다|마치도록\s*하겠습니다|'
        r'이상\s*질의를?\s*마치|이것으로\s*끝)'), 0.55),
    ("서술", re.compile(
        r'(이[가]\s*(있|되|된|없)|것으로\s*(보입니다|판단|생각|알고)|'
        r'되어\s*있습니다|하고\s*있습니다|인\s*것으로\s*알고|'
        r'부분이\s*있|상황입니다|실정입니다|형편입니다|'
        r'것입니다|바입니다|때문입니다|'
        r'라고\s*(봅니다|생각|판단|사료)|'
        r'인\s*것\s*같|라고\s*할\s*수\s*있)'), 0.5),
]
